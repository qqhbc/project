		...
		this.control({
			...
			"salesordergrid":{
				itemdblclick:function(grid, record, item, index, e, eOpts){
					...
					var url = "salesorder/show_salesorderdetail.htm?id="+id;
					...
					if(records[0].data.status){
						tit = "<font color=red>已审核(收货)</font>"
					}else{
						tit = "<font color=green>未审核(送货中)</font>"
					}
					...
					if(records[0].data.nullify){
						var btn = form.down("button[ref=nullify]");
						btn.disable(true);
						btn.setText("已作废");
					}else{
			...
			"saleitemform button[ref=check]" : {
				click : function(btn) {
					...
					var url = "salesorder/check_salesorder.htm";
			...
			/**form的作废*/
			"saleitemform button[ref=nullify]" : {
				click : function(btn) {
					var form = btn.up("saleitemform");
					var grid = form.up("salesorderlayout").down("salesordergrid");
					...
					//成功就返回
					var id = form.getForm().findField("id").getValue();
					var url = "salesorder/nullify_salesorder.htm";
					Ext.MessageBox.confirm("重要提示",
							"是否要作废吗？",
							function(e){
								if(e == 'yes'){
